...
description: Systematically checks and enforces TeX format and style consistency across all note files in the N/ directory. Use when verifying note formatting after manual editing, or when performing batch audits of the notes directory.
---

# TeX Consistency Check & Fix

This skill audits the `N/` directory for adherence to the project's strict LaTeX style, utilizing automated scripts and manual verification.
...
./.agent/skills/check_tex_consistency/scripts/audit_notes.py N/

# Run with --fix to apply changes
./.agent/skills/check_tex_consistency/scripts/audit_notes.py N/ --fix

# Run with --verbose for detailed output
./.agent/skills/check_tex_consistency/scripts/audit_notes.py N/ --verbose
```
...
2. **Header**: Ensures `\section{Notes X...}` matches the filename `NX.tex`.
...
1.  **Main Index Check**:
    -   Read `MTS.tex`.
    -   Ensure **ALL** populated note files in `N/` are `\input` in the main file.
