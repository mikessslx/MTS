...
Description:
    Audits N/NX.tex files for format, structure, style, and indentation.
...
def audit_file(filepath, fix=False):
    ...
    match = re.match(r'N(\d+)\.tex', filename)
    if not match:
        return []
...
def main():
    ...
    files = sorted(
        [f for f in os.listdir(args.directory) if re.match(r'N\d+\.tex', f)],
        key=lambda x: int(re.search(r'N(\d+)', x).group(1))
    )
    
    if not files:
        print(f"  = No N*.tex files found in {args.directory}")
        return
