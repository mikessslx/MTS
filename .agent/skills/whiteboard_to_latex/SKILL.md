...
1.  **Context Check**: Confirm the user has created the target file (e.g., `N/N26.tex`) with the basic `\section{Notes X...}` header. If not, ask them to, or do it if explicitly requested.
...
Use `replace_file_content` (or `write_to_file` if overwriting) to populate the target `N/NX.tex` file.

### 4. Integration & Verification

1.  **Update Index**:
    -   Run `scripts/update_main_index.py` to ensure the new file is `\input` in `MTS.tex`.
    -   ```bash
...
2.  **Verify**:
    ...
    -   Report back to the user: "Populated Notes X. Added to MTS.tex."
